...
const API_URL = 'https://api.coingecko.com/api/v3/simple/price?ids=tether&vs_currencies=eth';
...
const App: React.FC = () => {
  ...
  const handleChangeAction = (event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    <div className={css.container}>
      <h1>USDT/ETH <br />
        Price Calculator</h1>
      <input className={css.inputPrice} type="number" value={ethAmount} onChange={handleChangeEthAmount} placeholder="Enter ETH amount" />
      <select  className={css.selectActions} value={action} onChange={handleChangeAction}>
        <option value="buy">Buy ETH</option>
        <option value="sell">Sell ETH</option>
      </select>
      <button className={css.button} onClick={calculate}>Calculate</button>
      {error && <div>{error}</div>}
      {usdtPrice &&  <div className={css.containerResult}>
